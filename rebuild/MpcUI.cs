    ...
    internal static class MpcCharacterSlots
    {
        ...
        private static string StoragePath
        {
            get
            {
                ...
                return Path.Combine(dir, "MPCCharacterSlots.txt");
            }
        }
        ...
        public static void EnsureSelectedFromExisting()
        {
            lock (Sync)
            {
                ...
                if (_selectedSlot >= 0 && !string.IsNullOrWhiteSpace(Names[_selectedSlot]))
                    return;

                string existing = null;
                try
                {
                    existing = LocalPlayerState.GetDisplayName();
                }
                catch
                {
                }

                if (!string.IsNullOrWhiteSpace(existing) && !string.Equals(existing, "Player", StringComparison.OrdinalIgnoreCase))
                {
                    Names[0] = existing.Trim();
                    _selectedSlot = 0;
                    SaveLocked();
                }
            }
        ...
        public static void Select(int slot)
        {
            lock (Sync)
            {
                ...
                _selectedSlot = slot;
                if (!string.IsNullOrWhiteSpace(Names[slot]))
                {
                    try
                    {
                        LocalPlayerState.SetDisplayName(Names[slot]);
                    }
                    catch
                    {
                    }
        ...
        public static string GetName(int slot)
        {
            lock (Sync)
            {
                ...
                if (slot < 0 || slot >= SlotCount || string.IsNullOrWhiteSpace(Names[slot]))
                    return "Empty Slot";
        ...
        private static void EnsureLoadedLocked()
        {
            ...
            try
            {
                ...
                if (File.Exists(path))
                {
                    ...
                    {
                        string value = lines[i] == null ? null : lines[i].Trim();
                        Names[i] = string.IsNullOrWhiteSpace(value) ? null : value;
                    }

                    if (lines.Length > SlotCount)
                    {
                        int selected;
                        if (int.TryParse(lines[SlotCount], out selected) && selected >= 0 && selected < SlotCount)
        ...
        private static void SaveLocked()
        {
            try
            {
                ...
                List<string> lines = new List<string>(SlotCount + 1);
                for (int i = 0; i < SlotCount; i++)
                    lines.Add(Names[i] ?? string.Empty);
                lines.Add(_selectedSlot.ToString());
    ...
    public sealed class MultiplayerCampaignVM : ViewModel
    {
        ...
        public string IpAddress
        {
            ...
            set
            {
                ...
                _ipAddress = value;
                OnPropertyChangedWithValue(value, nameof(IpAddress));
            }
        }
        ...
        public string PlayerName
        {
            ...
            set
            {
                ...
                _playerName = value;
                OnPropertyChangedWithValue(value, nameof(PlayerName));
            }
        }
        ...
        public string CharacterName
        {
            ...
            set
            {
                ...
                OnPropertyChangedWithValue(value, nameof(CharacterName));
            }
        }
        ...
        public bool ShowMain
        {
            ...
            private set
            {
                if (_showMain == value) return;
                _showMain = value;
                OnPropertyChangedWithValue(value, nameof(ShowMain));
            }
        }
        ...
        public bool ShowCharacter
        {
            ...
            private set
            {
                if (_showCharacter == value) return;
        ...
        public bool ShowCreate
        {
            ...
            private set
            {
                if (_showCreate == value) return;
                _showCreate = value;
                OnPropertyChangedWithValue(value, nameof(ShowCreate));
            }
        }
        ...
        public bool ShowJoin
        {
            ...
            private set
            {
                if (_showJoin == value) return;
                _showJoin = value;
                OnPropertyChangedWithValue(value, nameof(ShowJoin));
            }
        }
        ...
        public void ExecuteCreateCharacter()
        {
            ...
            LocalPlayerState.SetDisplayName(name);
            _playerName = name;
            OnPropertyChangedWithValue(_playerName, nameof(PlayerName));
            RefreshCharacterBindings();
    ...
    public static class InitialMenuPatch
    {
        ...
        private static void OpenMultiplayerCampaign()
        {
            ScreenManager.PushScreen(
                ViewCreatorManager.CreateScreenView<MultiplayerCampaignScreen>());
        }
    }
